...
from tf_trainer.tf_gru_attention_multiclass_warmstart import model as tf_gru_attention_multiclass
...
tf.app.flags.DEFINE_string("warmstart_dir", None,
                           help="Path to the model to warmstart from.")
...
def main(argv):
  ...
  model_tf = tf_gru_attention_multiclass.TFRNNModel_warmstart(
    target_labels=dataset.labels())
  model = preprocessor.add_embedding_to_model(model_tf,
                                              base_model.TOKENS_FEATURE_KEY)

  trainer = model_trainer.ModelTrainer(dataset, model, FLAGS.warmstart_dir)
